...
class ScriptManager {
  ...
  async getOfficialScripts() {
    try {
      const officialScripts = defaultEditions.filter(edition => edition.isOfficial);
      const additionalScripts = await this.readScriptsFromDirectory(this.officialPath);
  ...
  async readScriptsFromDirectory(directoryPath) {
    try {
      ...
      const relevantKeys = keys.filter(key => key.startsWith(`script_${directoryKey}`));
  ...
  async loadScript(scriptId, type = 'custom') {
    try {
      ...
      if (type === 'official') {
        const officialScript = defaultEditions.find(edition => edition.id === scriptId);
        if (officialScript) {
        ...
        reader.onload = async (e) => {
          try {
            ...
            if (!validation.isValid) {
              reject(new Error(`剧本数据验证失败: ${validation.errors.join(', ')}`));
              return;
            }
