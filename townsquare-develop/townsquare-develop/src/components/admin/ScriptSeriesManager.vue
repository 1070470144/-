<template>
  <div class="series-manager">
      ...
      >
        <div class="series-header">
          ...
          <div class="series-meta">
            <span class="version-count">{{ series.versions.length }} 个版本</span>
            <span class="latest-version">最新: v{{ getLatestVersion(series) }}</span>
          </div>
        </div>
        ...
        <div class="versions-list">
          ...
          >
            ...
            <div class="version-actions">
              <button @click="editVersion(version)" class="edit-btn">编辑</button>
              <button @click="setAsLatest(version)" v-if="!version.isLatest" class="latest-btn">设为最新</button>
              <button @click="deleteVersion(version)" class="delete-btn">删除</button>
            </div>
          </div>
        </div>

        <div class="series-actions">
          <button @click="addVersion(series)" class="add-version-btn">添加版本</button>
          <button @click="editSeries(series)" class="edit-series-btn">编辑系列</button>
          <button @click="deleteSeries(series)" class="delete-series-btn">删除系列</button>
        </div>
      </div>
    </div>
      ...
      <div class="modal" @click.stop>
        ...
        <div class="modal-content">
          <div class="form-group">
            <label>系列名称</label>
            <input v-model="newSeries.name" type="text" placeholder="例如：暗流涌动" />
          </div>
          <div class="form-group">
            <label>描述</label>
            <textarea v-model="newSeries.description" placeholder="系列描述"></textarea>
          </div>
          <div class="form-group">
            <label>分类</label>
    ...
    <!-- 添加版本模态框 -->
    <div v-if="showVersionModal" class="modal-backdrop" @click="closeVersionModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
        ...
        <div class="modal-content">
          <div class="form-group">
            <label>版本号</label>
            <input v-model="newVersion.version" type="text" placeholder="例如：2.0" />
          </div>
          <div class="form-group">
            <label>版本描述</label>
